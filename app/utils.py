...
def load_queue_config(config_file: str = "../messaging_config.json") -> tuple[Dict[str, RabbitExchange], Dict[str, QueueConfig]]:
...
def create_dynamic_class(name: str, raw_data: Dict, return_type: type) -> type:
    class DynamicClass:
        def __getattr__(self, attr: str) -> return_type:
            ...
            raise AttributeError(f"'{name}' has no attribute '{full_name}'. Available options: {available}")
